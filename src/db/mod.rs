...
pub fn create_task(connection: &SqliteConnection, title: &str) -> Result<(), String> {
    ...
    match diesel::insert_into(schema::task::table)
        ...
        Err(err) => match err {
            ...
            _ => Err(String::from("An error occurred while attempting to create the task"))
        }
    }
}
...
pub fn query_tasks_by_title(connection: &SqliteConnection, title: &str) -> Result<models::Task, String> {
    match schema::task::table.filter(schema::task::title.eq(title)).first::<models::Task>(connection) {
        Ok(task) => Ok(task),
        Err(e) => match e {
